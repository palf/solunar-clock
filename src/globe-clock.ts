...
(async function(): Promise<void> {
  ...
  const slicesG = svg.append("g").attr("id", "slices");
  // Labels layer
  const labelsG = svg.append("g").attr("id", "labels");
  // Center mark layer
  const centerG = svg.append("g").attr("id", "center");
  // Hands layer (top - most visible)
  const handG = svg.append("g").attr("id", "hand");
  ...
  clockFace.drawTimezoneLabels(labelsG);
  // 4. Center mark
  ...
  const sunHand = handG.append("line")
    .attr("class", "hand-sun")
  ...
  const moonHand = handG.append("line")
  ...
  function updateHands(): void {
    ...
    sunHand.attr("x1", state.centerX)
           .attr("y1", state.centerY)
           .attr("x2", sunX2)
           .attr("y2", sunY2);
    moonHand.attr("x1", state.centerX)
            .attr("y1", state.centerY)
            .attr("x2", moonX2)
            .attr("y2", moonY2);
    ...
    existing.enter()
      ...
      .text(`Time: ${timeStr} (${state.timeSpeedMultiplier.toFixed(2)}x speed) | Center: ${state.centerLat.toFixed(1)}°, ${state.centerLon.toFixed(1)}°`);
  }
